...
import { Lock, Eye, EyeOff, ArrowRight, Palette, Star, Shield, CheckCircle } from 'lucide-react';
...
const ResetPasswordPage = () => {
  ...
  useEffect(() => {
    // In a real app, you would get this from URL params
    // For demo purposes, we'll simulate it
    const urlParams = new URLSearchParams(window.location.search);
    const tokenFromUrl = urlParams.get('token') || window.location.pathname.split('/').pop();
  ...
  const handleResetPassword = async (e) => {
    ...
    try {
      const response = await axios.post(`/user/resetPassword?token=${token}`, {
                    ...
                    <div className="space-y-6 mb-8">
                      <div className="flex items-center space-x-4 bg-white/10 backdrop-blur-sm rounded-lg p-4">
                        ...
                        <div className="text-left">
                          ...
                          <p className="text-sm text-gray-300">Strong password protection</p>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center space-x-4 bg-white/10 backdrop-blur-sm rounded-lg p-4">
                        ...
                        <div className="text-left">
                          ...
                          <p className="text-sm text-gray-300">Quick and secure process</p>
                        </div>
                      </div>
                    </div>
                      ...
                      <div className="space-y-6">
                        ...
                        {!token && (
                          <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                            <p className="text-yellow-700 text-sm">Invalid or missing reset token. Please check your email link.</p>
                          </div>
                        )}
                        ...
                        <div className="bg-gray-50 rounded-lg p-4">
                          ...
                          <ul className="text-xs text-gray-600 space-y-1">
                            <li className={`flex items-center ${resetData.password.length >= 8 ? 'text-green-600' : ''}`}>
                              <div className={`w-1.5 h-1.5 rounded-full mr-2 ${resetData.password.length >= 8 ? 'bg-green-500' : 'bg-gray-400'}`}></div>
                              At least 8 characters
                            </li>
                            <li className={`flex items-center ${/[A-Z]/.test(resetData.password) ? 'text-green-600' : ''}`}>
                              <div className={`w-1.5 h-1.5 rounded-full mr-2 ${/[A-Z]/.test(resetData.password) ? 'bg-green-500' : 'bg-gray-400'}`}></div>
                            ...
                            <li className={`flex items-center ${/\d/.test(resetData.password) ? 'text-green-600' : ''}`}>
                              <div className={`w-1.5 h-1.5 rounded-full mr-2 ${/\d/.test(resetData.password) ? 'bg-green-500' : 'bg-gray-400'}`}></div>
                              One number
                            </li>
                          </ul>
                        </div>
                        ...
                        <button
                          ...
                          disabled={!resetData.password || !resetData.confirmPassword || resetData.password !== resetData.confirmPassword || isLoading || !token}
